...
A data frame with one row per quantity: `prob` (or one `prob[<group>]`
row per group for a `prob ~ group` fit), `delay_mean` and `delay_sd`,
...
For a `prob ~ group` fit the CFR varies by group, so one `prob[<group>]`
